...
export function CreateTestCaseModal({ isOpen, onClose, updateTests }) {
    const { id } = useParams();
    ...
    function handleCreateTC() {
        ...
        data.append('file', input, `in-${id}-${name}-${uuid}`);

        data.append('file', output, `out-${id}-${name}-${uuid}`);
